...
(function (global, factory) {
  ...
  setTimeout(function () {
    return $('body').addClass('mdui-loaded');
  });
  var mdui = {
    $: $,
  };
  ...
  each(['val', 'html', 'text'], function (nameIndex, name) {
    ...
    function get($elements) {
      ...
      if (nameIndex === 0 && $(firstElement).is('select[multiple]')) {
        return map($(firstElement).find('option:checked'), function (element) {
      ...
      return this.each(function (i, element) {
        ...
        if (nameIndex === 0 && Array.isArray(computedValue)) {
          ...
          if ($(element).is('select[multiple]')) {
            map($(element).find('option'), function (option) {
  ...
  mdui.mutation = function (selector, apiInit) {
    ...
    $(selector).each(function (i, element) {
      return mutation(selector, apiInit, i, element);
    });
  };
  ...
  CollapseAbstract.prototype.open = function open(item) {
    ...
    $content
      ...
      .transitionEnd(function () {
        return this$1.transitionEnd($content, $item);
      });
  ...
  CollapseAbstract.prototype.openAll = function openAll() {
    ...
    this.getItems().each(function (_, element) {
      return this$1.open(element);
    });
  };
  /**
   * 关闭所有面板项
  ...
  var Fab = function Fab(selector, options) {
    ...
    if (this.options.trigger === 'click') {
      this.$btn.on(startEvent, function () {
        return this$1.open();
      });
    }
    // 触摸屏幕其他地方关闭快速拨号
  ...
  var Tab = function Tab(selector, options) {
    ...
    $window.on('resize', $.throttle(function () {
      return this$1.setIndicatorPosition();
    }, 100));
    // 监听点击选项卡事件
  ...
  Menu.prototype.closeSubMenu = function closeSubMenu($submenu) {
    ...
    $submenu
      ...
      .transitionEnd(function () {
        return $submenu.removeClass('mdui-menu-closing');
      })
      // 移除激活状态的样式
      .parent('.mdui-menu-item')
    ...
    $submenu.find('.mdui-menu').each(function (_, menu) {
      ...
      $subSubmenu
        ...
        .addClass('mdui-menu-closing')
        .transitionEnd(function () {
          return $subSubmenu.removeClass('mdui-menu-closing');
        })
        .parent('.mdui-menu-item')
        .removeClass('mdui-menu-item-active');
    });
        ...
        if (eventType === 'mouseover') {
          if ($submenu.length) {
            ...
            timeout = timeoutOpen = setTimeout(function () {
              return that.openSubMenu($submenu);
        ...
        // 鼠标移出菜单项时，关闭菜单项下的子菜单
          ...
          if ($submenu.length) {
            ...
            timeout = setTimeout(function () {
              return that.closeSubMenu($submenu);
            }, that.options.subMenuDelay);
